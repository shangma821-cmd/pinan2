...
test('home page reconstruction', async ({ page }) => {
  ...
  await expect(page.getByTestId('home-hero').getByText('频安科技')).toBeVisible();
  await expect(page.getByTestId('home-hero').getByText('守护生命节律')).toBeVisible();
...
test('about page reconstruction', async ({ page }) => {
  ...
  await expect(page.getByTestId('about-strategy').getByRole('heading', { name: /智慧健康管理/ })).toBeVisible();
  await expect(page.getByTestId('about-qualifications').getByRole('heading', { name: /技术.*与规范/ })).toBeVisible();
  await expect(page.getByTestId('about-qualifications')).toContainText('两项技术专利布局');
  await expect(page.getByTestId('about-patent-docs').getByRole('link', { name: /大数据能力证书/ })).toHaveAttribute('href', /pinan-big-data-certificate\.pdf$/);
  await expect(page.getByTestId('about-patent-docs').getByRole('link', { name: /云健康管理发明专利/ })).toHaveAttribute('href', /pinan-cloud-health-patent\.pdf$/);
  ...
  const foundingCard = page
    .getByTestId('about-timeline')
    .locator('.landing-about-timeline-card')
    .filter({ hasText: '公司成立' });
  await expect(foundingCard).toContainText('2025');
});
...
test('franchise page reconstruction', async ({ page }) => {
  ...
  await expect(page.getByText('全程服务支持，陪伴门店成长')).toBeVisible();
  await expect(page.getByText('3项支持，稳健合作')).toBeVisible();
...
test('brand identity matches legacy', async ({ page }) => {
  ...
  await expect(page.getByTestId('landing-nav')).toContainText('健康平安');
  await expect(page.getByTestId('landing-footer')).toContainText('PINAN Technology');
});
